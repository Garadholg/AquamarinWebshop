...
$(document).ready(function () {
    ...
    function removeItem(pressedElement) {
        ...
        $.ajax({
            type: "DELETE",
            url: "/Aquamarin-Webshop/cart?method=single&id=" + id,
            success: function()
            {
                row.remove();
            }
    ...
    function updateItem(pressedElement, action) {
        ...
        if (!isNaN(qty) && qty > 0) {
            $.ajax({
            type: "PUT",
            url: "/Aquamarin-Webshop/cart?id=" + id + "&action=" + action,
            success: function()
            {
                if (action === "decr") {
    ...
    $('.btnClearCart').click(function() {        
        $.ajax({
            type: "DELETE",
            url: "/Aquamarin-Webshop/cart?method=all",
            success: function()
            {
                $(".table > tbody").empty();
    ...
    $('.btnBuy').click(function(){
        window.location.href='/Aquamarin-Webshop/checkout';
    });
